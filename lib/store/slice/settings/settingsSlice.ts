...
type SettingsAction<T> = { value: T } & UpdateOption;

const initialState = () => {
  const { settings } = getStorageSettings({ resetOnInvalid: true });
...
export const settingsSlice = createSlice({
  ...
  reducers: (create) => ({
    setTheme: create.reducer((state, actions: PayloadAction<SettingsAction<Theme>>) => {
      const { value, setStorage, updateDocument } = actions.payload;
...
export const { setTheme, setFontSize, setSettings, setIncrementFontSize, setDecrementFontSize } = settingsSlice.actions;
